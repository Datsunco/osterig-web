...
export default class Store {
    ...
    setDeliveryPointRes(deliveryPointRes) {
        this.deliveryPointRes = deliveryPointRes
    }
    ...
    sliceSelectedParam(param) {
        ...
        this.seletedParams.forEach(element => {
            ...
            if (element.name != param.name) {
                changed.push({ id: element.id, name: element.name, type: element.type })

            }
            else {
                flag = true
                console.log("elemo", { id: element.id, name: element.name, type: element.type }, { id: param.id, name: param.name })
            }
        });
    ...
    async getTariffs(address) {
        try {
            ...
            if (resp){
                this.setDeliveryType('HOME')
                this.setDeliveryAddress(address)
                this.setDeliveryPrice(resp.data.tariff_codes.find((item) => item.tariff_code  === 139).delivery_sum)
                this.setDeliveryPointRes({})
            }
        } catch (e) {
            console.log(e)
        }
    ...
    async getTariffByPoint(address, type) {
        try {
            const resp = await DeliveryService.getTariffByPoint(address, type)
            this.setDeliveryPointRes(resp.data)
            this.setDeliveryType(resp.data.tariff_code == 138 ? 'PVZ' : 'POSTOMAT');
            this.setDeliveryPrice(resp.data.delivery_sum)
            this.setDeliveryAddress(address)
            this.setTariffs({})
            console.log(resp)
            // this.setTariffs(resp.data.tariff_codes)
    ...
    async getFeaturesByAddress(address) {
        try {
            const resp = await DeliveryService.getFeaturesByAddress(address)

            const features = resp?.data.features?.map((point) => ({
                type: 'Feature',
                id: point.code,
                geometry: {
                    type: 'Point',
                ...
                properties: {
                    ...
                    <p><button style="height: 40px; width: 100px; background: #000; color: #FFF; fontSize: 16px" data-id="${point.code}" data-type="${point.type}" data-adr="${point.location.address_full}">Выбрать </button></p>
                    <p><strong>Тип</strong>: ${point?.type}</p>
                    <p><strong>Часы работы</strong>: ${point.work_time}</p>
                    <p><strong>Комменатрий</strong>: ${point.address_comment}</p>
                    </p>`,
                    hintContent: point.name,
                },
            }));

            this.setPoints(features)
            this.setCenter(resp?.data.cords)
            console.log('pointss', resp)
            return { features, cords: resp.data.cords }
            // this.setTariffs(resp.data.tariff_codes)
        } catch (e) {
            console.log(e)
        }
    ...
    async getDeliveryPoints(center) {
        try {
            const resp = await DeliveryService.getDeliveryPoints(center)
            console.log('pointss', resp)
            return resp.data
        } catch (e) {
            console.log(e)
