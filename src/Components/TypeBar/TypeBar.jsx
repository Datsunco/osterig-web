      ...
      <div class="pop" style={{padding: isMobile ? '0 10px' : '0px', marginLeft: 'auto', marginRight: 'auto', display: 'flex', justifyContent: 'center'}}>
        {device.types.map((type, i) =>
          <a className="test" onClick={() => navigate(type.link)}>
            <ListGroupItem
              className="est"
              style={{ cursor: 'pointer', width: isMobile ? '130px' : '280px', height: isMobile ? '206px' : '315px' , backgroundImage: type.id === 8 ? 'url('+ popular+')' : '', backgroundSize: 'cover', color: type.id === 8 ? 'black' : ''}}
              key={type.id}
            >
